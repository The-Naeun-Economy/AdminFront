...
function AdminMain() {
    ...
    const initializePage = async (token) => {
        try {
            ...
            const [totalUsersData, yearsData, monthlyDataForYear, genderData] = await Promise.all([
    ...
    const setupAutoRefresh = (token) => {
        const interval = setInterval(async () => {
            try {
                const [totalUsersData, genderData] = await Promise.all([
                    apiCall("/admin/users/count", token, "총 유저 수를 갱신하는 데 실패했습니다."),
                    apiCall("/admin/users/gender/count", token, "성별 데이터를 갱신하는 데 실패했습니다."),
                ]);
                setState((prevState) => ({
                    ...prevState,
                    totalUsers: totalUsersData.totalCount,
