...
function AdminCommunityPosts() {
    ...
    return (<div className="admin-layout">
        ...
        <div className="admin-content">
            ...
            {!loading && userDetails && (<div className="user-details-container">
                <h2>게시글 관리</h2>
            </div>)}

            {!loading && (<div className="table-container">
                {posts.length > 0 ? (<table className="custom-table">
                    <thead>
                    <tr>
                        <th>제목</th>
                        <th>내용</th>
                    ...
                    >
                        <td>{post.title.length > 20 ? `${post.title.slice(0, 10)}...` : post.title}</td>
                        <td>
                        ...
                        <td>
                            <button
                                ...
                                onClick={(e) => {
                                    ...
                                    openCommentsModal(post.id); // 댓글 모달 열기
                                }}
                            >
                                댓글 보기
                            </button>
                ...
                </table>) : (<p className="no-results">게시글이 없습니다.</p>)}
            </div>)}
                            ...
                            <ul className="comments-list">
                                {comments.map((comment) => (
                                    <li key={comment.id} className="comment-item">
                                        <p>
                                            <strong>작성자:</strong> {comment.userNickname}{" "}
                                            <strong>유저 아이디:</strong> {comment.userId}
                                        </p>
                                        <p>
                                            <strong>작성일:</strong>{" "}
            ...
            {/* 게시글 상세 모달 */}
            {postDetails && (
                <div className="modal-overlay" onClick={closePostDetailsModal}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        <button className="close-button" onClick={closePostDetailsModal}>
                            &times;
                        </button>
                        <h2>게시글 상세 정보</h2>
                        <p style={{ marginBottom: "20px" }}>
                            <strong>제목:</strong> {postDetails.title}
                        </p>
                        <p style={{ marginBottom: "20px" }}>
                            <strong>내용:</strong> {postDetails.content}
                        </p>
                        <p><strong>카테고리:</strong> {postDetails.category}</p>
                        <p><strong>작성일:</strong> {new Date(postDetails.createdAt).toLocaleString()}</p>
                        <p><strong>좋아요:</strong> {postDetails.likesCount}</p>
                        <p><strong>조회수:</strong> {postDetails.viewCount}</p>
                    </div>
                </div>
            )}
        </div>
